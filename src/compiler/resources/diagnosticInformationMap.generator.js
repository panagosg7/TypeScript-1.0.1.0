...
fs.readFile('diagnosticMessages.json', 'utf8', function (err, data) {
  if (err) {
  ...
  console.log('/// <reference path="..\\core\\diagnosticCategory.ts" />')
  console.log("module TypeScript {")
  console.log("		export var diagnosticInformationMap: IIndexable<any> = {")

  for (var key in obj) {

    var newkey = key.replace(/[\+\[\]\<\>?\.,-\/#!$%\^&\*;:{}=\-_`~()\'\ \"]/g, "_")			//special chars -> _
                                                            .replace(/_+/g, "_")
                                                    .replace(/\*/g, "Asterisk")
                                                    .replace(/\//g, "Slash")
                                                            .replace(/_$/g, "")
    ...
    if (obj[key].category === 'Error') console.log('  { "code": ' + obj[key].code + ', "category": DiagnosticCategory.Error },')
